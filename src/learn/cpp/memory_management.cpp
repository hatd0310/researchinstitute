...
/*
        ...
        aligned_alloc	allocates the specified number of bytes at the specified
   alignment realloc			increases or decreases the size of the
   specified block of memory, moving it if necessary calloc
   allocates the specified number of bytes and initializes them to zero free
...
int main() {
  int *data = new int;
  int *data =
      (int *)malloc(sizeof(int)); // notice the use of sizeof for portability
}