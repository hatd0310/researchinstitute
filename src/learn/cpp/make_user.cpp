...
void write_to_file(const std::string& path, const std::vector<std::string>& list_users)
{
...
std::vector<std::string> active_users(const std::vector<std::string>& list_users)
{
...
std::vector<std::string> inactive_users(const std::vector<std::string>& list_users)
{
	std::vector<std::string> result;
	for (const std::string& user : list_users)
	{
...
std::vector<std::vector<std::string>> split_string(const std::vector<std::string>& list_users, char delimiter)
{
...
std::vector<std::string> update_group_role(const std::vector<std::string>& list_users)
{
	...
	for (int i = 0; i < parts.size(); i++)
	{
		...
		result.push_back(CMD + "-user=\"" + parts[i][0] + "\" -group=\"" + parts[i][1] + "\" -role=\"" + parts[i][2] + "\"");
	}
...
std::vector<std::string> update_volume(const std::vector<std::string>& list_users)
{
	std::vector<std::vector<std::string>> parts = split_string(list_users, ',');
	std::vector < std::string> result;
	...
	{
		std::cout << parts[i][0] << parts[i][1] << parts[i][2] << std::endl;
		result.push_back(CMD + "-user=\"" + parts[i][0] + "\" -defaultvolume=\"" + parts[i][1] + "\" -defaultlocalvolume=\"" + parts[i][2] + "\"");
	}
...
std::vector<std::string> update_license_server(const std::vector<std::string>& list_users)
{
	std::vector<std::vector<std::string>> parts = split_string(list_users, ',');
	std::vector < std::string> result;
	...
	{
		std::cout << parts[i][0] << parts[i][1] << parts[i][2] << std::endl;
		result.push_back(CMD + "-user=\"" + parts[i][0] + "\" -licenseserver=\"" + parts[i][1] + "\" -licenselevel=\"" + parts[i][2] + "\"");
	}
